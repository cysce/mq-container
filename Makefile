...
define build-mq
	...
	$(if $(findstring docker,$(COMMAND)), @docker run --rm --name $(BUILD_SERVER_CONTAINER) --network build --network-alias build --volume $(DOWNLOADS_DIR):/opt/app-root/src:ro --detach docker.io/nginx nginx -g "daemon off;",)
...
.PHONY: log-build-env
log-build-vars:
	$(info $(SPACER)$(shell printf $(TITLE)"Build environment - ************ log-build-vars ************"$(END)))
	@echo ARCH=$(ARCH)
...
install-build-deps:
	$(info $(SPACER)$(shell printf $(TITLE)"************ install-build-deps ************"$(END)))
